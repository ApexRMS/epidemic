...
{
    partial class DistributionValueDataFeedView : DataFeedView
    {
        ...
        protected override void InitializeView()
        {
            ...
            DataSheet ds = this.Project.GetDataSheet(Shared.DISTRIBUTION_TYPE_DATASHEET_NAME);

            try
            {
                this.m_TypeId = ds.ValidationTable.GetValue(Shared.DISTRIBUTION_TYPE_NAME_UNIFORM_INTEGER);
            }
            catch (Exception)
            {
                Debug.Assert(false);
        ...
        public override void LoadDataFeed(Core.DataFeed dataFeed)
        {
            ...
            v.LoadDataFeed(dataFeed, Shared.DISTRIBUTION_VALUE_DATASHEET_NAME);
        ...
        {
            if (e.ColumnIndex == this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_DIST_TYPE_ID_COLUMN_NAME].Index)
            {
                ...
                DataGridViewComboBoxCell Cell = (DataGridViewComboBoxCell)dgv.Cells[Shared.DISTRIBUTION_VALUE_DIST_TYPE_ID_COLUMN_NAME];

                Cell.DataSource = dv;
                Cell.ValueMember = ds.ValueMember;
                Cell.DisplayMember = ds.DisplayMember;
            }
            else if (e.ColumnIndex == this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_VALUE_DIST_TYPE_ID_COLUMN_NAME].Index)
            {
                DataGridViewRow dgv = this.m_Grid.Rows[e.RowIndex];
                DataSheet ds = this.Project.GetDataSheet(Shared.DISTRIBUTION_TYPE_DATASHEET_NAME);
                string filter = CreateKnownDistributionTypeFilter(this.Project);
                DataView dv = new DataView(ds.GetData(), filter, ds.DisplayMember, DataViewRowState.CurrentRows);
                DataGridViewComboBoxCell Cell = (DataGridViewComboBoxCell)dgv.Cells[Shared.DISTRIBUTION_VALUE_VALUE_DIST_TYPE_ID_COLUMN_NAME];
        ...
        private void OnGridCellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_DIST_TYPE_ID_COLUMN_NAME].Index)
            {
                DataGridViewRow dgv = this.m_Grid.Rows[e.RowIndex];
                DataGridViewComboBoxCell Cell = (DataGridViewComboBoxCell)dgv.Cells[Shared.DISTRIBUTION_VALUE_DIST_TYPE_ID_COLUMN_NAME];
                DataGridViewComboBoxColumn Column = (DataGridViewComboBoxColumn)this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_DIST_TYPE_ID_COLUMN_NAME];
            ...
            else if (e.ColumnIndex == this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_VALUE_DIST_TYPE_ID_COLUMN_NAME].Index)
            {
                DataGridViewRow dgv = this.m_Grid.Rows[e.RowIndex];
                DataGridViewComboBoxCell Cell = (DataGridViewComboBoxCell)dgv.Cells[Shared.DISTRIBUTION_VALUE_DIST_TYPE_ID_COLUMN_NAME];
                DataGridViewComboBoxColumn Column = (DataGridViewComboBoxColumn)this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_VALUE_DIST_TYPE_ID_COLUMN_NAME];
        ...
        private void OnGridCellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (e.ColumnIndex != this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_VALUE_DIST_MIN_COLUMN_NAME].Index &&
                e.ColumnIndex != this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_VALUE_DIST_MAX_COLUMN_NAME].Index)
            {
                return;
            }
            ...
            DataGridViewCell TypeCell = this.m_Grid.Rows[e.RowIndex].Cells[Shared.DISTRIBUTION_VALUE_VALUE_DIST_TYPE_ID_COLUMN_NAME];
        ...
        {
            if (e.ColumnIndex == this.m_Grid.Columns[Shared.DISTRIBUTION_VALUE_VALUE_DIST_TYPE_ID_COLUMN_NAME].Index)
            {
        ...
        private static List<int> GetInternalDistributionTypeIds(Project project)
        {
            ...
            DataSheet ds = project.GetDataSheet(Shared.DISTRIBUTION_TYPE_DATASHEET_NAME);

            foreach (DataRow dr in ds.GetData().Rows)
            {
                if (dr.RowState != DataRowState.Deleted)
                {
                    if (Booleans.BoolFromValue(dr[Shared.DISTRIBUTION_TYPE_IS_INTERNAL_COLUMN_NAME]))
                    {
